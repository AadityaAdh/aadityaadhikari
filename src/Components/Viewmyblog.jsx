...
export default function Viewmyblog() {
  ...
  useEffect(() => {
    async function getSinglePosts() {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:4005/blogs/getBlog/${id}`, {
          method: "GET",
        ...
        if (response.ok) {
          ...
          console.log(singleitem.blog)
          setdata(singleitem.blog);
          // parse


          const parser = new DOMParser();
          const doc = parser.parseFromString(singleitem.blog.content, 'text/html');
          ...
          const blockquoteMatches = [
            ...singleitem.blog.content.matchAll(/<blockquote>(.*?)<\/blockquote>/gs)
  ...
  async function handledelete(id) {
    const response = await fetch(`http://localhost:4005/blogs/deleteBlog/${id}`, {
      method: "DELETE",
  ...
  return (
    <div className='viewmyprojectoutercontainer'>
      {data ? (
        <div>
          {data.image && (<img src={`http://localhost:4005/${data.image}`}></img>)}

          <h1 className='viewmyprojecttitle'>{data.title}</h1>
          
          <p className='viewmyprojectsummary'>{data.summary}</p>
          <br></br>
          {isadmin && (<div className='viewmyprojectbuttoncontainer'><button onClick={() => handleupdate(data._id)}>Update this blog</button><button onClick={() => handledelete(data._id)}>Delete this blog</button></div>)}
          <br></br>
          ...
          <div className='viewmyprojectdowncontainer'>
            ...
            <div className='tableofcontentscontainer'>
              ...
              <h2 className='viewmyprojecttableofcontents'>Table of contents</h2>
